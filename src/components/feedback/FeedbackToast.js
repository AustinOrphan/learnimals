...
class FeedbackToast extends BaseComponent {
  ...
  constructor(options) {
    super({
      ...
      cssClasses: [
        'feedback-toast',
        `feedback-toast--${options.type || 'success'}`,
        `feedback-toast--${options.character || 'max'}`,
      ],
      ...options,
    });
  ...
  getTypeIcon(type) {
    const icons = {
      ...
      progress:
        '<span class="feedback-icon feedback-icon--progress" aria-label="Progress">📈</span>',
      achievement:
        '<span class="feedback-icon feedback-icon--achievement" aria-label="Achievement">🏆</span>',
    };
