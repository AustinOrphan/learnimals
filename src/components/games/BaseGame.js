...
export default class BaseGame {
  ...
  trackGameEnd() {
    const accuracy =
      this.analytics.questionsAnswered > 0
  ...
  checkAchievements() {
    ...
    if (this.analytics.currentStreak >= 10) {
      this.achievementSystem.checkAchievement('streak_10', {
  ...
  playFeedbackAudio(soundId, character, type) {
    ...
    this.loadCharacterAudio(character)
      .then(() => {
    ...
    const _reactions = {
      ...
      error: {
        ...
        zara: [
          ...
          'Discovery needs tries! 🔍',
        ],
        aria: [
          'Art is about expression! 🎭',
          'Every brushstroke counts! 🖌️',
          'Creativity flows freely! 🌊',
        ],
        codecat: [
          'Debugging is learning! 🔧',
          'Code evolves with fixes! 📝',
      ...
      progress: {
        ...
        max: [
          'Math skills advancing! 📊',
          'Problem-solving improved! 🎯',
          'Calculation power up! ⚡',
        ],
        zara: [
          'Scientific thinking grows! 🧪',
          'Research skills develop! 📋',
          'Discovery abilities rise! 🚀',
        ],
        aria: ['Artistic vision expands! 👁️', 'Creative skills bloom! 🌸', 'Imagination soars! 🦋'],
        codecat: [
          'Coding prowess increases! 💻',
          'Programming logic sharpens! 🔪',
          'Debug skills level up! 🐛',
        ],
      },
      achievement: {
  ...
  animateCharacterReaction(character, type, emoji, reaction) {
    ...
    const characterElement = this.container?.querySelector(
      `.${character}-character, .character-avatar, #${character}-character`
    );
  ...
  cleanupFeedback(feedbackId) {
    ...
    const element = this.feedbackState.feedbackContainer?.querySelector(
