...
class GameProgressDashboard {
  ...
  renderGameCards() {
    ...
    games.forEach(game => {
      const analytics = this.progressTracker.gameAnalytics[game.id] || this.progressTracker.getDefaultGameAnalytics();
      const cardData = {
        title: game.name,
  ...
  renderRecentAchievements() {
    ...
    if (recent.length === 0) {
      recentList.innerHTML = '<p class="no-achievements">Start playing games to unlock achievements!</p>';
    } else {
  ...
  renderGameDetailedStats() {
    const analytics = this.progressTracker.gameAnalytics[this.selectedGame] || this.progressTracker.getDefaultGameAnalytics();
  ...
  renderAllGamesStats() {
    ...
    chartsContainer.appendChild(this.createComparisonChart('Time Played', 'totalTimePlayed', this.formatTime));
    
    // Sessions played comparison
    chartsContainer.appendChild(this.createComparisonChart('Sessions Played', 'sessionsPlayed'));
    
    // Accuracy comparison
    chartsContainer.appendChild(this.createComparisonChart('Accuracy', 'accuracy', (val) => `${Math.round(val)}%`));
  ...
  createComparisonChart(title, metric, formatter = (val) => val) {
    ...
    const games = ['word-scramble', 'number-line-jump', 'element-match', 'sentence-builder', 'color-palette'];
    const maxValue = Math.max(...games.map(g => 
      this.progressTracker.gameAnalytics[g]?.[metric] || 0
    ));
  ...
  renderSkillsProgress(analytics) {
    ...
    return Object.entries(analytics.skillProgress).map(([skill, data]) => `
  ...
  getCrossGameAchievements() {
    return Object.values(this.progressTracker.achievements).filter(a => 
      !a.id.includes('-') || a.id.startsWith('cross-')
    );
  }
