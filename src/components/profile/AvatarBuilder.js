...
class AvatarBuilder {
        ...
        onClick: () => {
          if (unlocked) {
            if (selected) {
              ...
              const sameCategory = this.currentConfig.accessories.filter(a => 
                AVATAR_ACCESSORIES[a].category === accessory.category
              );
              
              if (accessory.category === 'head' && sameCategory.length > 0) {
                // Replace head accessory
                this.currentConfig.accessories = this.currentConfig.accessories.filter(a => 
                  AVATAR_ACCESSORIES[a].category !== 'head'
                );
              }
  ...
  createColorElement(options) {
    ...
    if (options.special && options.hex === 'rainbow') {
      colorStyle = 'background: linear-gradient(45deg, #FF0000, #FF7F00, #FFFF00, #00FF00, #0000FF, #4B0082, #9400D3);';
    } else {
  ...
  saveAvatar() {
    if (!this.currentConfig.isValid(this.profile)) {
      Modal.show({
        ...
        content: '<p>Your avatar configuration contains locked items. Please remove them before saving.</p>',
        buttons: [
