...
class UserProgress {
  constructor() {
    ...
    this.achievementsList = [
      { id: 'math_novice', name: 'Math Novice', description: 'Complete 5 math lessons', icon: 'medal-bronze', unlocked: false, progress: 0, target: 5 },
      { id: 'math_expert', name: 'Math Expert', description: 'Answer 50 math questions correctly', icon: 'medal-silver', unlocked: false, progress: 0, target: 50 },
      { id: 'math_master', name: 'Math Master', description: 'Reach level 5 in math', icon: 'medal-gold', unlocked: false, progress: 0, target: 5 },
      { id: 'reading_explorer', name: 'Reading Explorer', description: 'Read 10 stories', icon: 'book-bronze', unlocked: false, progress: 0, target: 10 },
      { id: 'word_wizard', name: 'Word Wizard', description: 'Learn 100 new words', icon: 'wand-silver', unlocked: false, progress: 0, target: 100 },
      { id: 'science_discoverer', name: 'Science Discoverer', description: 'Complete 5 science experiments', icon: 'flask-bronze', unlocked: false, progress: 0, target: 5 },
      { id: 'art_creator', name: 'Art Creator', description: 'Complete 5 art projects', icon: 'palette-bronze', unlocked: false, progress: 0, target: 5 },
      { id: 'learnimals_friend', name: 'Learnimals Friend', description: 'Visit the site for 5 consecutive days', icon: 'star-bronze', unlocked: false, progress: 0, target: 5 },
      { id: 'bubble_popper', name: 'Bubble Popper', description: 'Score 20 points in Bubble Pop', icon: 'bubble-silver', unlocked: false, progress: 0, target: 20 },
      { id: 'word_master', name: 'Word Master', description: 'Complete 15 words in Word Scramble', icon: 'words-gold', unlocked: false, progress: 0, target: 15 }
    ];
  ...
  updateMathProgress(data) {
    ...
    this.userData.progress.math.level = Math.floor(this.userData.progress.math.lessonsCompleted / 3) + 1;
  ...
  updateReadingProgress(data) {
    if (data.bookId) {
      ...
      if (data.bookData && data.bookData.completed &&
        ...
        this.updateAchievementProgress('reading_explorer', this.userData.progress.reading.storiesRead);
      }
    }
    ...
    this.userData.progress.reading.level = Math.floor(this.userData.progress.reading.storiesRead / 2) + 1;
  ...
  updateScienceProgress(data) {
    ...
    if (data.experimentCompleted) {
      ...
      this.updateAchievementProgress('science_discoverer', this.userData.progress.science.experimentsCompleted);
    }
    ...
    if (data.quizScore) {
      ...
      this.userData.progress.science.quizScore =
        (oldTotal + data.quizScore) / topics;
    }

    // Calculate level based on experiments completed (1 level per 2 experiments)
    this.userData.progress.science.level = Math.floor(this.userData.progress.science.experimentsCompleted / 2) + 1;
  ...
  updateArtProgress(data) {
    ...
    this.userData.progress.art.level = Math.floor(this.userData.progress.art.projectsCompleted / 2) + 1;
  ...
  updateGameStats(game, stats) {
    ...
    if (game === 'bubblePop') {
      ...
      this.updateAchievementProgress('word_master', this.userData.gameStats.wordScramble.wordsCompleted);
    }
  ...
  getProgressSummary() {
    return {
      ...
      stats: {
        ...
        mathAccuracy: this.userData.progress.math.questionsAnswered > 0
          ? Math.round((this.userData.progress.math.correctAnswers / this.userData.progress.math.questionsAnswered) * 100)
          : 0,
  ...
  resetUserData() {
    ...
    const {name} = this.userData.profile;
