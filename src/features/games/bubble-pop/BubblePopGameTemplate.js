...
export default class BubblePopGameTemplate extends BaseGame {
    ...
    spawnBubbles() {
        ...
        for (let i = 0; i < this.settings.bubbleCount; i++) {
            const x = spacing * (i + 1);
            const y = baseY + getRandomInt(-20, 20); // Add some variation
            let answer;
    ...
    createBubbleBackground(radius) {
        ...
        if (!this.bubbleCache.has(cacheKey)) {
            ...
            gradient.addColorStop(0, this.themeColors.secondary + '80');
    ...
    update(deltaTime, timestamp) {
        ...
        // Check if correct bubble was missed
        const correctBubble = this.bubbles.find(b => b.isCorrect);
        if (!correctBubble || !correctBubble.active) {
    ...
    renderQuestion() {
        ...
        this.ctx.roundRect(bgX, bgY, bgWidth, bgHeight, 10);
        this.ctx.fill();
    ...
    onResize(width, height) {
        ...
        // Reposition bubbles if they exist
