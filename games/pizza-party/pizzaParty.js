...
export default class PizzaPartyGame {
  ...
  positionAvailableSlices() {
    ...
    const sliceSize = 60;
    const spacing =
      (this.canvas.width - sliceSize * this.gameState.availableSlices.length) /
  ...
  drawCompletionIndicator() {
    ...
    const targetValue =
      this.gameState.targetFraction.numerator / this.gameState.targetFraction.denominator;
  ...
  handlePointerDown(event) {
    ...
    const clickedSlice = this.gameState.availableSlices.find(slice => {
      const distance = Math.sqrt(Math.pow(x - slice.x, 2) + Math.pow(y - slice.y, 2));
  ...
  handlePointerMove(event) {
    ...
    if (this.draggedSlice) {
      ...
      const distanceToCenter = Math.sqrt(
        Math.pow(x - this.config.pizza.centerX, 2) + Math.pow(y - this.config.pizza.centerY, 2)
      );
  ...
  checkSolution() {
    ...
    if (
      ...
      if (
        currentFraction.numerator * targetFraction.denominator >
  ...
  showSuccessAnimation() {
    // Confetti particles
    this.particles.addConfetti(this.config.pizza.centerX, this.config.pizza.centerY, 30);
