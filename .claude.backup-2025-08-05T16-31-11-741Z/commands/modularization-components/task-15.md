# modularization-components - Task 15

Execute task 15 for the modularization-components specification.

## Task Description
Migrate math subject handler in src/features/subjects/math/math.js

## Code Reuse
**Leverage existing code**: existing math.js educational workflows
...
```
Use the spec-task-executor agent to implement task 15: "Migrate math subject handler in src/features/subjects/math/math.js" for the modularization-components specification.
...
3. Implement ONLY task 15: "Migrate math subject handler in src/features/subjects/math/math.js"
...
npx @pimzino/claude-code-spec-workflow@latest get-tasks modularization-components 15 --mode single
...
Task details:
- ID: 15
- Description: Migrate math subject handler in src/features/subjects/math/math.js
- Leverage: existing math.js educational workflows
- Requirements: 1.1, 2.1, 4.1
```

**Fallback Execution**: If the agent is not available, you can execute:
```
/spec-execute 15 modularization-components
```
...
2. Execute task 15: "Migrate math subject handler in src/features/subjects/math/math.js"
...
1. **Mark task complete**: Use the get-tasks script to mark completion:
   ```bash
   npx @pimzino/claude-code-spec-workflow@latest get-tasks modularization-components 15 --mode complete
   ```
2. **Confirm to user**: State clearly "Task 15 has been marked as complete"
...
```
Use the spec-task-implementation-reviewer agent to review the implementation of task 15 for the modularization-components specification.
...
- Specific task 15 details via get-tasks
- All implementation changes for task 15
...
```
Use the spec-duplication-detector agent to analyze code duplication for task 15 of the modularization-components specification.
...
```
Use the spec-integration-tester agent to test the implementation of task 15 for the modularization-components specification.
