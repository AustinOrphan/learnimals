...
describe('Comprehensive Security Tests', () => {
  ...
  test('Content Security Policy enforcement', async () => {
    ...
    cspTests.forEach(test => {
      const isAllowed = trustedSources.some(
        source => test.source === source || test.source.startsWith(source)
      );
  ...
  test('DOM manipulation security', async () => {
    ...
    eventHandlers.forEach(handler => {
      ...
      if (handler.safe) {
        ...
        console.log(
          `   ❌ Malicious event handler blocked: ${handler.event} -> ${handler.handler}`
        );
      }
    });
  ...
  test('File upload security validation', async () => {
    ...
    fileUploads.forEach(file => {
      ...
      if (file.safe) {
        console.log(
          `   ✓ File upload accepted: ${file.name} (${file.type}, ${(file.size / 1024).toFixed(1)}KB)`
        );
      } else {
  ...
  test('Privacy and data protection compliance', async () => {
    ...
    Object.entries(userData).forEach(([category, data]) => {
      ...
      console.log(
        `   📊 Data classified: ${category} -> ${privacyLevel} (consent: ${classification.requiresConsent})`
      );
    });
