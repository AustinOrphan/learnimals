...
// Enhanced timer utilities for comprehensive timer control with RAF support
const TimerUtils = {
  ...
  // Run all pending timers immediately (including RAF)
  runAllTimers: () => {
    vi.useFakeTimers();
    vi.runAllTimers();
    vi.useRealTimers();
...
describe('BaseGame Feedback System', () => {
  ...
  describe('Timer and Memory Management', () => {
    it('should track and clear all setTimeout calls', async () => {
      // Create multiple feedbacks that use timers
      await game.showFeedback('success', 'Test message 1');
      await game.showFeedback('error', 'Test message 2');
      await game.showFeedback('hint', 'Test message 3');
