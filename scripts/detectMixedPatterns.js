...
const MIXED_PATTERNS = {
  typeofModule: {
    regex: /if\s*\(\s*typeof\s+module\s*!==?\s*['"]undefined['"]\s*(?:&&\s*module\.exports)?\s*\)/gi,
...
class MixedPatternDetector {
  ...
  async scanFile(filePath) {
    try {
      ...
      const hasES6 = fileAnalysis.patterns.some(p => 
        ['es6Export', 'es6Import'].includes(p.name)
      );
      
      fileAnalysis.hasMixedPatterns = hasCommonJS && hasES6;
      fileAnalysis.hasGlobalPatterns = fileAnalysis.patterns.some(p => 
        p.name === 'globalAssignment'
      );
  ...
  async generateSummary() {
    ...
    const sortedFiles = [...this.results.mixedPatternFiles].sort((a, b) => 
  ...
  async outputResults() {
    ...
    if (this.results.summary.mixedPatternFiles > 0) {
      ...
      this.results.summary.migrationPriority.forEach((file, index) => {
        console.log(`   ${index + 1}. ${file.path} (score: ${file.severityScore}, effort: ${file.estimatedEffort})`);
      });
