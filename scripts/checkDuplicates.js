...
async function main() {
  ...
  Object.entries(groups).forEach(([original, dups]) => {
    if (original === 'no-original') {
      ...
      dups.forEach(dup => {
        const status = dup.duplicateHash === dup.originalHash ? 
  ...
  if (process.argv.includes('--fix')) {
    console.log(`${colors.yellow}⚠️  Fix mode: This will delete duplicate files that are identical to originals.${colors.reset}`);
    ...
    process.stdin.once('data', () => {
      ...
      if (duplicates.length - deleted > 0) {
        console.log(`${colors.yellow}${duplicates.length - deleted} file(s) require manual review (different content).${colors.reset}`);
      }
      
      process.exit(0);
    });
  } else {
    console.log(`${colors.blue}Run with --fix flag to automatically remove identical duplicates.${colors.reset}`);
