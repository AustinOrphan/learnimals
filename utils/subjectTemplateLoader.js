...
class SubjectTemplateLoader {
  ...
  static async loadTemplate(options) {
    try {
      // Fetch the template
      const response = await fetch('/templates/subject.html');
      if (!response.ok) {
      ...
      template = template.replace(
        ...
        escapeHTML(options.subjectDescription)
      );
      ...
      template = template.replace(
        /{{heroSubtitle}}/g,
        escapeHTML(options.heroSubtitle || characterGreeting)
      );
      ...
      if (options.enableCharacterRenderer !== false) {
        const characterRendererCSS = `
          <link rel="stylesheet" href="/styles/components/CharacterPreviewRenderer.css" />`;
      ...
      if (options.enableCharacterRenderer !== false) {
        const characterImport = `
          <script type="module">
            import CharacterPreviewRenderer from '/character-generation/ui/CharacterPreviewRenderer.js';
  ...
  static createEnhancedOptions(subjectKey, customOptions = {}) {
    ...
    const defaultOptions = {
      subjectName:
        character.personality?.favoriteSubject?.charAt(0).toUpperCase() +
      ...
      characterType:
        character.species.primary.charAt(0).toUpperCase() + character.species.primary.slice(1),
